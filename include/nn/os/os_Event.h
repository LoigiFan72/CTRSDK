...
public:
    ...
    { 
        this->EventBase::Initialize(manualReset ? RESET_TYPE_STICKY: RESET_TYPE_ONESHOT); 
    }
    void Finalize() 
    { 
        this->EventBase::Finalize(); 
    }
    void Signal() 
    { 
    ...
    void Wait() 
    { 
        this->EventBase::WaitOne(); 
    }
    bool Wait(nn::fnd::TimeSpan timeout) 
    { 
        return this->EventBase::WaitOne(timeout); 
    }
};
}
}